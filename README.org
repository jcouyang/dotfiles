...
第一个 [[http://dotfiles.github.io/][dotfiles]] 指将 HOME 目录下的 =.XXX= (它们是真的字面意思dotfiles) 文件同步到 Github 上的方式。
...
=config.fish= 被链到了我创建的 =dotfiles/fish= 底下同样的目录中。

这样其实每次修改 =config.fish= 都实际上是在修改 =~/dotfiles=  下的 =config.fish=
